...
title: HMM Scalable
layout: project
tags: [bayesian, open-source, big data]
image: ../assets/img/projects/hmm_scalable.png
description: An open-source command line toolkit for creating Hidden Markov Models at scale. Handles billion-record datasets in minutes. Supports standard and original models.
---

Code: [GitHub](https://github.com/myudelson/hmm-scalable)
...
2. $p-learn$ or$ p(T)$ - is a probability the skill will transition into _mastered_ state after a practice attempt,
3. $p-forget$ or $p(F)$ - is a probability that the skill will transition into _not mastered state_ after a practice attempt. Traditionally, $p-forget$ is set to zero and is not counted towards the total number of parameters.
...
BKT parameters can be represented in the vector/matrix form. A vector of priors – $\Pi$ – contains the $p-init$ parameter. There are N elements in the priors vector ($N=2$ in standard BKT). Throughout this discussion, we will we assume that $p-init$ is the first element of the $\Pi$ vector. $p-learn$ is part of the transitions matrix A that captures probabilities of mastery state changes and its size is $N\times N$. No forgetting is captured by setting $A[1,2] = 0$ (from mastered to unmastered). $p-learn$ corresponds to $A[2,1]$ - from unmastered to mastered. $p-guess$ and $p-slip$ are specified in the observation matrix B that is $N \times M$, where M is the number of observations (M=2 in standard BKT). The first column corresponds to a correct observation, the second – to the incorrect. In standard BKT with two observations, $p-guess$ is $B[2,1]$ – unmastered skill but a correct response, and $p-splip$ is $B[1,2]$ - mastered skill but an incorrect response. 
...
`-u` : upper boundaries for params, comma-separated for priors, transition, and emission probabilities (without skips); default is `1,1,1,1,1,1,1,0.3,0.3,1` with $p-slip$ and $p-guess$ capped at 0.3.
...
If we fit BKT model using Gradient Descent method using `-s 1.2` argument, the recovered parameters would be: p`Lo=0.00000000, pT=0.3095828498, pS=0.18955371, pG=0.16195590`, the accuracy would remain at 90% while RMSE = 0.344145. Loglikelihood changes from  9.3763477 to 8.0992564 after 19 iterations.
